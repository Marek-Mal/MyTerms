...
class put_sell_games(generics.UpdateAPIView):
    ...
    def put(self, request, pk=0, *args, **kwargs):
        try:
            ...
            if (int(get_user_id_from_token) == int(qs_check_if_user_posted)) == int(request.data['user']):
                self.update(request, *args, **kwargs)
                return Response("Zmieniono Pomyślnie")
            else:
...
class put_sell_acc(generics.UpdateAPIView):
...
class put_coach(generics.UpdateAPIView):
...
class put_boosting(generics.UpdateAPIView):
